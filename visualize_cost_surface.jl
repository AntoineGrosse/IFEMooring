...
for t_idx in time_indices
    ...
    for (i, γ₀) in enumerate(γ₀_range)
        for (j, γ₁) in enumerate(γ₁_range)
            ...
            try
                ...
                @show X_dofs = (Xvec,)
                A_dofs = SVector(γ₀, γ₁)
                cost_val = VALUE(straincost(X_dofs, nothing, A_dofs, t_val))
                costs[i, j] = cost_val
    ...
    axislegend(ax)
    save("figs/cost_surface_max_disp_$(max_disp)_t_$(t_idx).png", fig)
    ...
    axislegend(ax)
    save("figs/cost_vs_γ₀_max_disp_$(max_disp)_t_$(t_idx).png", fig)
